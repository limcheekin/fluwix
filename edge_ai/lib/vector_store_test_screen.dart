...
import 'package:edge_ai/models/embedding_model.dart' as model_config;
import 'package:edge_ai/services/embedding_download_service.dart';
...
class _VectorStoreTestScreenState extends State<VectorStoreTestScreen> {
  ...
  Future<void> _runAllTests() async {
    ...
    await _runTest(
        'Test 5: Search Performance (Real Embeddings)', _testSearchPerformance);
  ...
  Future<void> _testBasicVectorStore() async {
    ...
    for (final result in results) {
      _log(
          '  - ${result.content} (similarity: ${result.similarity.toStringAsFixed(4)})');
    }
    ...
    final statsAfterClear =
        await FlutterGemmaPlugin.instance.getVectorStoreStats();
  ...
  Future<void> _testDynamicDimensions() async {
    ...
    final testModels = [
      model_config.EmbeddingModel
          .gecko64, // Gecko: 110M params, seq=64, 110MB (FASTEST)
      model_config.EmbeddingModel
    ...
    for (final modelConfig in testModels) {
      ...
      initStopwatch.stop();
      _log(
          '   ⏱️  Initialization time: ${initStopwatch.elapsedMilliseconds}ms');

      // Generate real embedding and measure time
      _log('🧪 Generating test embedding...');
      final embedStopwatch = Stopwatch()..start();
      final testEmbedding =
          await embeddingModel.generateEmbedding('Test short query for search');
      embedStopwatch.stop();
      final actualDimension = testEmbedding.length;
      _log(
          '   ⏱️  Embedding generation time: ${embedStopwatch.elapsedMilliseconds}ms');
      ...
      _log(
          '✅ Generated ${actualDimension}D embedding (seq_len=${modelConfig.maxSeqLen})');
      ...
      if (stats.vectorDimension != actualDimension) {
        throw Exception(
            'Expected ${actualDimension}D in stats, got ${stats.vectorDimension}D');
      }

      _log(
          '✅ ${modelConfig.displayName} test passed! (${actualDimension}D vectors)');
  ...
  Future<void> _testStorageOptimization() async {
    ...
    for (int i = 0; i < 100; i++) {
      final embedding = List.generate(
          _modelDimension, (j) => (i + j) / _modelDimension.toDouble());
      await FlutterGemmaPlugin.instance.addDocumentWithEmbedding(
        id: 'doc_$i',
    ...
    final expectedMaxSize =
        (expectedWithOverheadKB * 1.5).toInt(); // 50% buffer
  ...
  Future<void> _testSearchPerformance() async {
    ...
    gemmaStopwatch.stop();
    final gemmaAvgMs =
        gemmaStopwatch.elapsedMilliseconds / searchQueries.length;

    _log('✅ EmbeddingGemma 256 results:');
    _log('   Average: ${gemmaAvgMs.toStringAsFixed(2)}ms per query');
    _log(
        '   Total: ${gemmaStopwatch.elapsedMilliseconds}ms for ${searchQueries.length} queries');
    _log(
    ...
    geckoStopwatch.stop();
    final geckoAvgMs =
        geckoStopwatch.elapsedMilliseconds / searchQueries.length;

    _log('✅ Gecko 64 results:');
    _log('   Average: ${geckoAvgMs.toStringAsFixed(2)}ms per query');
    _log(
        '   Total: ${geckoStopwatch.elapsedMilliseconds}ms for ${searchQueries.length} queries');
    _log(
    ...
    _log(
        '┌────────────────────────────────────────────────────────────────────┐');
    _log(
        '│                    EmbeddingGemma 256     │  Gecko 64              │');
    _log(
        '├────────────────────────────────────────────────────────────────────┤');
    _log(
        '│ Model size:        179MB                  │  110MB                 │');
    _log(
        '│ Parameters:        300M                   │  110M                  │');
    _log(
        '│ Max seq length:    256 tokens             │  64 tokens             │');
    _log(
        '├────────────────────────────────────────────────────────────────────┤');
    _log(
    ...
    if (speedup > 1) {
      _log(
          '🚀 Gecko 64 is ${speedup.toStringAsFixed(1)}x FASTER than EmbeddingGemma 256!');
    } else if (speedup < 1) {
      _log(
          '🚀 EmbeddingGemma 256 is ${(1 / speedup).toStringAsFixed(1)}x FASTER than Gecko 64!');
    } else {
    ...
    if (speedup > 1.2) {
      ...
      _log(
          '   ✅ Use EmbeddingGemma 256 for accuracy (300M params, better quality)');
    } else if (speedup < 0.8) {
