...
import 'package:edge_ai/utils/test_preferences.dart';
import 'package:edge_ai/vector_store_test_screen.dart';
...
class _IntegrationTestScreenState extends State<IntegrationTestScreen> {
  ...
  final TextEditingController _customInferencePathController =
      TextEditingController();
  final TextEditingController _customEmbeddingModelPathController =
      TextEditingController();
  final TextEditingController _customEmbeddingTokenizerPathController =
      TextEditingController();
  ...
  static const embeddingTokenizerAssetPath =
  ...
  Future<void> _setDefaultPaths() async {
    ...
    if (Platform.isAndroid) {
      ...
      try {
        ...
        _customInferencePathController.text =
            '${documentsDir.path}/gemma3-270m-it-q8.task';
        _customEmbeddingModelPathController.text =
            '${documentsDir.path}/embeddinggemma-300M_seq1024_mixed-precision.tflite';
        _customEmbeddingTokenizerPathController.text =
            '${documentsDir.path}/sentencepiece.model';
      } catch (e) {
  ...
  Future<void> _checkInstalledModels() async {
    ...
    try {
      ...
      final inferenceModels = installedModels
          .where((id) => id.endsWith('.task') || id.endsWith('.bin'))
          .toList();
      ...
      final embeddingModels = installedModels
          .where((id) => id.endsWith('.tflite') || id.endsWith('.model'))
          .toList();
  ...
  Future<void> _testReplacePolicy() async {
    await _runTest('Replace + File/SetPath + LoRA', () async {
      ...
      _log('🔧 Testing Legacy setModelPath (migrated)...');
      try {
        await FlutterGemma.installModel(
          modelType: legacy.ModelType.gemmaIt,
        ).fromFile(externalFile.path).install();
        _log('✅ Legacy: setModelPath OK (migrated)');
      } catch (e) {
        _log('⚠️  setModelPath error: $e');
      }

      // === Test 4: LoRA Weights ===
      _log('🎯 Testing LoRA weights (migrated)...');
      try {
        ...
        await FlutterGemma.installModel(
          modelType: legacy.ModelType.gemmaIt,
        ).withLoraFromFile(loraFile.path).install();
        _log('✅ LoRA: Set weights OK (migrated)');

        // To delete LoRA weights, reinstall model without LoRA
        await FlutterGemma.installModel(
          modelType: legacy.ModelType.gemmaIt,
        ).fromFile(externalFile.path).install();
        _log('✅ LoRA: Delete weights OK (migrated)');
      } catch (e) {
        _log('⚠️  LoRA error: $e');
      }
  ...
  Future<void> _testInterruptAndRestart() async {
    ...
    if (testStage == 'after_restart') {
      ...
      await _testInterruptAndRestartPart2();
    } else {
      // Part 1: Initial run
      await _testInterruptAndRestartPart1();
    }
  }

  Future<void> _testInterruptAndRestartPart1() async {
  ...
  Future<void> _testInterruptAndRestartPart2() async {
  ...
  Future<void> _testBundledInferenceLegacyInternal() async {
  ...
  Future<void> _testBundledInferenceLegacy() async {
    await _runTest('Bundled Inference Model (Legacy)',
        _testBundledInferenceLegacyInternal);
  }

  /// Internal version without _runTest wrapper
  Future<void> _testBundledInferenceModernInternal() async {
  ...
  Future<void> _testBundledInferenceModern() async {
    await _runTest('Bundled Inference Model (Modern)',
        _testBundledInferenceModernInternal);
  }

  /// Internal version without _runTest wrapper
  Future<void> _testBundledEmbeddingLegacyInternal() async {
  ...
  Future<void> _testBundledEmbeddingLegacy() async {
    await _runTest('Bundled Embedding Model (Legacy)',
        _testBundledEmbeddingLegacyInternal);
  }

  /// Internal version without _runTest wrapper
  Future<void> _testBundledEmbeddingModernInternal() async {
  ...
  Future<void> _testBundledEmbeddingModern() async {
    await _runTest('Bundled Embedding Model (Modern)',
        _testBundledEmbeddingModernInternal);
  }

  // === NETWORK DOWNLOAD ===

  /// Internal version without _runTest wrapper
  Future<void> _testInferenceDownloadLegacyInternal() async {
  ...
  Future<void> _testInferenceDownloadLegacy() async {
    await _runTest('Download Inference Model (Legacy)',
        _testInferenceDownloadLegacyInternal);
  }

  /// Internal version without _runTest wrapper
  Future<void> _testInferenceDownloadModernInternal() async {
  ...
  Future<void> _testInferenceDownloadModern() async {
    await _runTest('Download Inference Model (Modern)',
        _testInferenceDownloadModernInternal);
  }

  // === ASSET LOADING ===

  /// Internal version without _runTest wrapper
  Future<void> _testInferenceFromAssetsLegacyInternal() async {
  ...
  Future<void> _testInferenceFromAssetsLegacy() async {
    await _runTest('Load Inference from Assets (Legacy)',
        _testInferenceFromAssetsLegacyInternal);
  }

  /// Internal version without _runTest wrapper (for use in _runAllTests)
  Future<void> _testInferenceFromAssetsModernInternal() async {
  ...
  Future<void> _testInferenceFromAssetsModern() async {
    await _runTest('Load Inference from Assets (Modern)',
        _testInferenceFromAssetsModernInternal);
  }
  ...
  Future<void> _setCustomInferencePathLegacy() async {
    await _runTest('Set Custom Inference Path (Legacy)', () async {
      ...
      // Legacy API replaced: setModelPath
      await FlutterGemma.installModel(
        modelType: legacy.ModelType.gemmaIt,
  ...
  /// Internal version without _runTest wrapper (for use in _runAllTests)
  Future<void> _testEmbeddingDownloadLegacyInternal() async {
  ...
  Future<void> _testEmbeddingDownloadLegacy() async {
    await _runTest('Download Embedding Model (Legacy)',
        _testEmbeddingDownloadLegacyInternal);
  }

  /// Internal version without _runTest wrapper (for use in _runAllTests)
  Future<void> _testEmbeddingDownloadModernInternal() async {
  ...
  Future<void> _testEmbeddingDownloadModern() async {
    await _runTest('Download Embedding Model (Modern)',
        _testEmbeddingDownloadModernInternal);
  }

  /// Internal version without _runTest wrapper (for use in _runAllTests)
  Future<void> _testEmbeddingFromAssetsLegacyInternal() async {
  ...
  Future<void> _testEmbeddingFromAssetsLegacy() async {
    await _runTest('Load Embedding from Assets (Legacy)',
        _testEmbeddingFromAssetsLegacyInternal);
  }

  /// Internal version without _runTest wrapper (for use in _runAllTests)
  Future<void> _testEmbeddingFromAssetsModernInternal() async {
  ...
  Future<void> _testEmbeddingFromAssetsModern() async {
    await _runTest('Load Embedding from Assets (Modern)',
        _testEmbeddingFromAssetsModernInternal);
  }
  ...
  Future<void> _runAllTests() async {
    ...
    try {
      ...
      setState(() {
        ...
        _runAllTestsProgress =
            'Failed: $_currentTestIndex/$_totalTests completed';
      });
    } finally {
      setState(() {
        _isTesting = false;
                    ...
                    children: [
                      ...
                      TextField(
                        ...
                        decoration: const InputDecoration(
                          hintText:
                              'hf_... (paste your HuggingFace token here)',
              ...
              children: [
                ...
                if (_hasPendingResumeTest)
                  _buildButton('Continue Resume Test',
                      _testInterruptAndRestartPart2, Colors.deepOrange),
                if (_interruptedUrl != null && _interruptedPath != null)
                  _buildButton(
